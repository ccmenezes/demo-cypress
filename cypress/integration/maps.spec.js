...
describe('Maps', ()=> {
    ...
    beforeEach(()=> {
        ...
        cy.title().should(
            'include',
            'Antes de continuar para o Google Maps'
        )
    ...
    it('Search a valid district', ()=> {
        ...
        cy.get(searchButton).click().then(()=> {

            cy.get(successSearchTitle).should('be.visible').invoke('text').then($value => {
        ...
        cy.get(directionIcon).should('be.visible').click({force:true}, {waitForAnimations:true})

        cy.log('->Validate the direction search box direction')
        cy.get(directionSearchBox).should('be.visible').then($el => {
